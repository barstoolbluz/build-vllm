# vLLM 0.14.0 for NVIDIA Blackwell DC (SM100: B100, B200, GB200) — AVX
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM100 + AVX)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "10.0";
  variantName = "vllm-python312-cuda12_9-sm100-avx";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.0 for NVIDIA B100/B200/GB200 (SM100) [CUDA 12.9, custom PyTorch AVX]";
      platforms = [ platform ];
    };
  })
