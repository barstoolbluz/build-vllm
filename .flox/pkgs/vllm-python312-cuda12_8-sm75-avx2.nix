# vLLM 0.14.1 for NVIDIA Turing (SM75: T4, RTX 2080 Ti) — AVX2
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM75 + AVX2)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "7.5";
  variantName = "vllm-python312-cuda12_8-sm75-avx2";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA T4/RTX 2080 Ti (SM75) [CUDA 12.8, custom PyTorch AVX2]";
      platforms = [ platform ];
    };
  })
