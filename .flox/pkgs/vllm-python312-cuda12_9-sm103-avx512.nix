# vLLM 0.14.0 for NVIDIA Blackwell Ultra (SM103: B300, GB300) — AVX-512
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM103 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "10.3";
  variantName = "vllm-python312-cuda12_9-sm103-avx512";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.0 for NVIDIA B300/GB300 (SM103) [CUDA 12.9, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
