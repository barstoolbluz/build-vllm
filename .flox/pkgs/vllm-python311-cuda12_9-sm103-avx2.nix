# vLLM 0.14.1 for NVIDIA Blackwell Ultra (SM103: B300, GB300) — AVX2
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM103 + AVX2)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "10.3";
  variantName = "vllm-python311-cuda12_9-sm103-avx2";
...
in
  python311Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA B300/GB300 (SM103) [CUDA 12.9, custom PyTorch AVX2]";
      platforms = [ platform ];
    };
  })
