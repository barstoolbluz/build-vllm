# vLLM 0.14.1 for NVIDIA Grace Hopper (SM90: GH200) — ARMv9
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM90 + ARMv9)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "9.0";
  variantName = "vllm-python312-cuda12_9-sm90-armv9";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA Grace Hopper GH200 (SM90) [CUDA 12.9, custom PyTorch ARMv9]";
      platforms = [ platform ];
    };
  })
