# vLLM 0.14.1 for NVIDIA Ampere DC (SM80: A100, A30) — AVX-512 BF16
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM80 + AVX-512 BF16)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.0";
  variantName = "vllm-python312-cuda12_8-sm80-avx512bf16";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA A100/A30 (SM80) [CUDA 12.8, custom PyTorch AVX-512 BF16]";
      platforms = [ platform ];
    };
  })
