# vLLM 0.14.0 for NVIDIA Ampere (SM86: RTX 3090, A40) — AVX-512
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM86 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.6";
  variantName = "vllm-python311-cuda12_8-sm86-avx512";
...
in
  python311Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.0 for NVIDIA RTX 3090/A40 (SM86) [CUDA 12.8, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
