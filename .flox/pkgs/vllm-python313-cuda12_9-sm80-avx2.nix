# vLLM 0.15.1 for NVIDIA Ampere DC (SM80: A100, A30) — AVX2
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM80 + AVX2)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.0";
  variantName = "vllm-python313-cuda12_9-sm80-avx2";
...
in
  python313Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.15.1 for NVIDIA A100/A30 (SM80) [CUDA 12.9, custom PyTorch AVX2]";
      platforms = [ platform ];
    };
  })
