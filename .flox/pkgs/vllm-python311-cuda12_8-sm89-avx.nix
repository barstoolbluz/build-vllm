# vLLM 0.14.1 for NVIDIA Ada Lovelace (SM89: RTX 4090, L4, L40) — AVX
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM89 + AVX)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.9";
  variantName = "vllm-python311-cuda12_8-sm89-avx";
...
in
  python311Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA RTX 4090/L4/L40 (SM89) [CUDA 12.8, custom PyTorch AVX]";
      platforms = [ platform ];
    };
  })
