# vLLM 0.14.1 for NVIDIA Ampere (SM86: RTX 3090, A40) — AVX2
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM86 + AVX2)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.6";
  variantName = "vllm-python311-cuda12_8-sm86-avx2";
...
in
  python311Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA RTX 3090/A40 (SM86) [CUDA 12.8, custom PyTorch AVX2]";
      platforms = [ platform ];
    };
  })
