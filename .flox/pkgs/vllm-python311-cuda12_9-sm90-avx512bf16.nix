# vLLM 0.14.0 for NVIDIA Hopper (SM90: H100, H200, L40S) — AVX-512 BF16
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM90 + AVX-512 BF16)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "9.0";
  variantName = "vllm-python311-cuda12_9-sm90-avx512bf16";
...
in
  python311Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.0 for NVIDIA H100/H200/L40S (SM90) [CUDA 12.9, custom PyTorch AVX-512 BF16]";
      platforms = [ platform ];
    };
  })
