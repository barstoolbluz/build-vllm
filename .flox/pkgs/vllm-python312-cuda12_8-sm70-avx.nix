# vLLM 0.14.1 for NVIDIA Volta (SM70: V100, Titan V) — AVX
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM70 + AVX)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "7.0";
  variantName = "vllm-python312-cuda12_8-sm70-avx";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA V100/Titan V (SM70) [CUDA 12.8, custom PyTorch AVX]";
      platforms = [ platform ];
    };
  })
