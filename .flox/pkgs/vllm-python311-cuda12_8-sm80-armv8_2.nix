# vLLM 0.14.1 for NVIDIA Ampere DC (SM80: A100 (Altra PCIe)) — ARMv8.2
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM80 + ARMv8.2)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.0";
  variantName = "vllm-python311-cuda12_8-sm80-armv8_2";
...
in
  python311Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA A100 Altra PCIe (SM80) [CUDA 12.8, custom PyTorch ARMv8.2]";
      platforms = [ platform ];
    };
  })
