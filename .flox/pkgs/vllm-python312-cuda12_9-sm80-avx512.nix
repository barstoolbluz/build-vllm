# vLLM 0.14.0 for NVIDIA Ampere DC (SM80: A100, A30) — AVX-512
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM80 + AVX-512)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.0";
  variantName = "vllm-python312-cuda12_9-sm80-avx512";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.0 for NVIDIA A100/A30 (SM80) [CUDA 12.9, custom PyTorch AVX-512]";
      platforms = [ platform ];
    };
  })
