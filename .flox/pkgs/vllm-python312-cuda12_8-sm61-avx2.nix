# vLLM 0.14.0 for NVIDIA Pascal (SM61: P40, GTX 1080 Ti) — AVX2
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM61 + AVX2)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "6.1";
  variantName = "vllm-python312-cuda12_8-sm61-avx2";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.0 for NVIDIA P40/GTX 1080 Ti (SM61) [CUDA 12.8, custom PyTorch AVX2]";
      platforms = [ platform ];
    };
  })
