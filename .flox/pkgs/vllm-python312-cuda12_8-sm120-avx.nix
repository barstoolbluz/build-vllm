# vLLM 0.14.1 for NVIDIA Blackwell (SM120: RTX 5090, RTX PRO 6000) — AVX
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM120 + AVX)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "12.0";
  variantName = "vllm-python312-cuda12_8-sm120-avx";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA RTX 5090/RTX PRO 6000 (SM120) [CUDA 12.8, custom PyTorch AVX]";
      platforms = [ platform ];
    };
  })
