# vLLM 0.14.1 for NVIDIA Hopper (SM90: H100, H200, L40S) — AVX2
# CUDA 12.8 — Requires NVIDIA driver 550+
# Custom PyTorch 2.9.1 built from source (SM90 + AVX2)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "9.0";
  variantName = "vllm-python312-cuda12_8-sm90-avx2";
...
in
  python312Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.1 for NVIDIA H100/H200/L40S (SM90) [CUDA 12.8, custom PyTorch AVX2]";
      platforms = [ platform ];
    };
  })
