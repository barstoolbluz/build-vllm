# vLLM 0.14.0 for NVIDIA Ampere (SM86: RTX 3090, A40) — AVX-512 BF16
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM86 + AVX-512 BF16)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "8.6";
  variantName = "vllm-python311-cuda12_9-sm86-avx512bf16";
...
in
  python311Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.14.0 for NVIDIA RTX 3090/A40 (SM86) [CUDA 12.9, custom PyTorch AVX-512 BF16]";
      platforms = [ platform ];
    };
  })
