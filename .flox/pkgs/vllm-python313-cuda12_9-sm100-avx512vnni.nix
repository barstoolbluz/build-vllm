# vLLM 0.15.1 for NVIDIA Blackwell DC (SM100: B100, B200, GB200) — AVX-512 VNNI
# CUDA 12.9 — Requires NVIDIA driver 560+
# Custom PyTorch 2.9.1 built from source (SM100 + AVX-512 VNNI)
{ pkgs ? import <nixpkgs> {} }:
let
  ...
  smCapability = "10.0";
  variantName = "vllm-python313-cuda12_9-sm100-avx512vnni";
...
in
  python313Custom.pkgs.vllm.overrideAttrs (oldAttrs: {
    ...
    NIX_BUILD_CORES = 16;
    meta = oldAttrs.meta // {
      description = "vLLM 0.15.1 for NVIDIA B100/B200/GB200 (SM100) [CUDA 12.9, custom PyTorch AVX-512 VNNI]";
      platforms = [ platform ];
    };
  })
