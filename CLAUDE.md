...
Unlike ORT builds, vLLM variants require no source/version/patch overrides — the nixpkgs vllm package (v0.13.0) already supports per-SM targeting, and torch is a pre-built binary wheel.
...
```nix
# vLLM 0.13.0 for NVIDIA Hopper (SM90: H100, H200, L40S)
# CUDA 12.9 — Requires NVIDIA driver 560+
```

The `meta.description` also includes the CUDA version:
```nix
description = "vLLM 0.13.0 for NVIDIA H100/H200/L40S (SM90) [CUDA 12.9]";
```
